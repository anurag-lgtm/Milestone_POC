...
mondayClient.setToken(process.env.MONDAY_API_KEY);
...
const LinkConnectedColumnThroughId = async (
  ...
  try {
   ...
   monday.setToken(
    "eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjU2MTk4MTAwOCwiYWFpIjoxMSwidWlkIjo4MTkyNjcwMiwiaWFkIjoiMjAyNS0wOS0xNVQxMDo0OToyNC4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6Mjk1NTY3MzcsInJnbiI6ImFwc2UyIn0.K9LP7Id84SX4U3mfW_zVd8UxauFI1Cw8nZ_z_evkwYM"
  );
...
async function getColumnValues(itemId) {
...
async function getPersonColumnValue(itemId,columnId) {
 ...
 query {
  ...
  `;
  const d = await mondayClient.api(mutation);
  console.log(d, "ddddd", mutation, "sad", d?.data?.items[0]);
...
const checkItemsExistsorNot = async (boardId, itemName) => {
  try {
    const monday = initMondayClient();
    monday.setToken("eyJhbGciOiJIUzI1NiJ9.eyJ0aWQiOjU2MTk4MTAwOCwiYWFpIjoxMSwidWlkIjo4MTkyNjcwMiwiaWFkIjoiMjAyNS0wOS0xNVQxMDo0OToyNC4wMDBaIiwicGVyIjoibWU6d3JpdGUiLCJhY3RpZCI6Mjk1NTY3MzcsInJnbiI6ImFwc2UyIn0.K9LP7Id84SX4U3mfW_zVd8UxauFI1Cw8nZ_z_evkwYM"
...
module.exports = {
  ...
  LinkConnectedColumnThroughId,getColumnValues,
  getColumnValueWithName,getSubItemsBoardId,getPersonColumnValue,getName
